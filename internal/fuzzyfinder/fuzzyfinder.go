...
var (
	...
	// ErrHotkey is returned with the highlighted item when the configured
	// hotkey is pressed.
	ErrHotkey  = errors.New("hotkey pressed")
	...
	for {
		...
		default:
			...
			case errors.Is(err, ErrAbort):
				return nil, ErrAbort
			case errors.Is(err, ErrHotkey):
				f.stateMu.RLock()
				defer f.stateMu.RUnlock()
				if len(f.state.matched) == 0 {
					return nil, ErrAbort
				}
				return []int{f.state.matched[f.state.y].Idx}, ErrHotkey
